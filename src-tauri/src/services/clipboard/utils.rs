...
pub fn parse_cleanup_rules(raw_rules: &str) -> Vec<CleanupRule> {
    let mut rules = Vec::new();
    let mut next_action = CleanupAction::Clean;
    ...
    for line in raw_rules.lines() {
        ...
        if line.starts_with('#') {
            ...
            if lower.contains("action: ignore") {
                next_action = CleanupAction::Ignore;
            } else if lower.contains("action: clean") {
                next_action = CleanupAction::Clean;
            }
            continue;
        }
        ...
        if let Some((pattern, replacement)) = line.split_once("=>") {
            ...
            // Reset action for next rule (default is clean)
            next_action = CleanupAction::Clean;
        }
    }
    rules
...
pub fn apply_cleanup_rules(text: &str, rules: &[CleanupRule]) -> CleanupResult {
    let mut current_text = text.to_string();

    for rule in rules {
        if rule.regex.is_match(&current_text) {
            if rule.action == CleanupAction::Ignore {
                return CleanupResult::Discard;
            }
            current_text = rule
                .regex
                .replace_all(&current_text, rule.replacement.as_str())
                .into_owned();
        }
    }

    CleanupResult::Changed(current_text)
}
...
pub fn truncate_html_for_preview(html: &str) -> Option<String> {
    ...
    if let Some(start) = start_pos {
        ...
        if end_rel == 0 {
            // Some Office/WPS table fragments omit closing </tr>. Returning the
            // intact fragment is safer than chopping through markup and showing
            // raw HTML in the preview.
...
pub fn detect_content_type(text: &str) -> String {
    ...
    if trimmed.starts_with("www.")
        || trimmed.contains("://")
            && trimmed.split("://").next().map_or(false, |scheme| {
                !scheme.is_empty()
                    && scheme
                        .chars()
...
// Windows-specific parse_cf_html removed for macOS refactor.

#[cfg(test)]
mod tests {
    use super::{
        attach_rich_image_fallback, build_entry_preview, contains_sensitive_info,
        derive_rich_text_content, detect_content_type, split_rich_html_and_image_fallback,
    ...
    #[test]
    fn rich_image_fallback_round_trips_html_and_payload() {
        let html = "<html><body><p>Hello</p></body></html>";
        let payload = "data:image/png;base64,abc123";

        let combined = attach_rich_image_fallback(html, payload);
        let (clean_html, extracted_payload) = split_rich_html_and_image_fallback(&combined);

        assert_eq!(clean_html, html);
        assert_eq!(extracted_payload.as_deref(), Some(payload));
    }

    #[test]
    fn detect_content_type_supports_custom_protocol_urls() {
        assert_eq!(detect_content_type("myapp+custom://open/page"), "url");
    }

    #[test]
    fn detect_content_type_rejects_invalid_scheme_prefix() {
        assert_eq!(detect_content_type("://foo"), "text");
    }

    #[test]
    fn contains_sensitive_info_detects_url_when_enabled() {
    ...
    #[test]
    fn contains_sensitive_info_detects_www_url_when_enabled() {
        assert!(contains_sensitive_info(
    ...
    #[test]
    fn contains_sensitive_info_skips_url_when_kind_disabled() {
        assert!(!contains_sensitive_info(
            "https://example.com",
            &["phone".to_string()],
            &[],
